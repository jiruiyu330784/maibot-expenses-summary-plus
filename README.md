...
nickname = "麦麦"       # 转发消息内的昵称；留空自动取机器人昵称
...
`report.nickname` 用于指定合并转发消息内各节点的昵称，留空时自动解析机器人昵称（多源回退），取不到则用默认名。设为与 QQ 端账号显示名一致即可保证转发内外观感统一。
...
- 新增 `/投喂史`（别名 `/历史投喂`、`/投喂记录`、`/donations`）命令，按日期时间展示历史全部投喂明细与合计。
- 修复合并转发消息节点昵称与账号昵称不一致：昵称解析改为多源回退，并新增 `report.nickname` 配置项显式指定。
- 修复 `/账本` 今日开销停留在上次财报快照：查询时实时刷新当日成本，不再依赖财报触发记账。
